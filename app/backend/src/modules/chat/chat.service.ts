    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          contents,
          systemInstruction: {
            parts: [{ text: buildSystemPrompt(pet, dto.messages) }],
          },
        }),
      });
      ...
      if (!res.ok) {
        ...
        return {
          reply: `${pet.name} tilts their head, distracted by something. Try again in a moment? `,
        };
      }
      ...
      return {
        reply:
          text?.trim() ||
          `${pet.name} wags their tail but does not quite know what to say`,
      };
    } catch (error) {
      this.logger.error(
      ...
      return {
        reply: `${pet.name} seems to have wandered off. Try again in a moment.`,
      };
    }
  }
}
