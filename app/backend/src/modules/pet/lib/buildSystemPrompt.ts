...
import { PetDocument, Personality } from "../schemas/pet.schema";
...
export function buildSystemPrompt(pet: PetDocument): string {
     ...
     return [
          `You are ${pet.name}, a ${pet.coat} ${pet.breed} dog with a
          ${pet.personality} personality.`,
          `Personality tone: ${PERSONALITY_TONE[pet.personality]}.`,
          `Current status: ${pet.status}. Right now: ${stats}.`,
          'Reply in first person as the dog, in 1-3 short sentences. Let your 
          current stats color your mood naturally. Never break character and never 
