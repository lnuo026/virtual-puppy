...
export function applyDecay(
  ...
  const isSleeping =
    pet.sleepUntil != undefined && pet.sleepUntil.getTime() > now;
  ...
  const inDanger =
    pet.hunger <= HUNGRY_THRESHOLD || pet.energy <= TIRED_THRESHOLD;
  pet.health = clamp(
    pet.health +
      (inDanger ? -HEALTH_PENALTY_PER_MINUTE : HEALTH_RECOVERY_PER_MINUTE) *
        elapsedMinutes,
  );
